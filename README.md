...
ciadpi --fake -1 --ttl 8
ciadpi -i 127.0.0.1 -p 1080 --disorder 1 --fake 0 --ttl 1 --auto=torst --tlsrec 1+s -x 1 --grease c,v,g,e --tls-split 50+s+e```

------
### Аутентификация SOCKS5

Программа поддерживает аутентификацию по логину и паролю (метод `0x02` USER/PASS).

- `--auth-file <путь>`
    Файл с учётными данными в формате `логин:пароль` (по одной записи на строку).
...
- `--auth <логин:пароль>`
     Задать одиночную пару логин/пароль прямо в командной строке.
      Имеет приоритет над `--auth-file`.

Если ни один из этих параметров не задан, аутентификация отключена.
------
...
-G, --grease <c,v,g,e>
    Применяет GREASE-подобные рандомизации к TLS Client Hello
    Можно комбинировать флаги через запятую:
        c - замена случайных cipher suites на зарезервированные значения GREASE
        v - замена версии TLS в расширении supported_versions
        g - замена supported groups на GREASE значения
        e - добавление/замена расширения с типом GREASE
    Пример: --grease c,g

-z, --tls-split <pos_t>
    Разделяет одну TLS-запись на две в указанной позиции
    В отличие от --tlsrec, который вставляет пустую запись, эта опция разрезает
    содержимое самой записи, создавая две валидные TLS-записи.
    Позиция задаётся аналогично --split (поддерживаются флаги +s, +h, +e, +m).
    Увеличивает общий размер запроса на 5 байт (добавляется заголовок второй записи).
    Пример: --tls-split 50+s+e  (разделить в конце SNI)
------
...
Добавляет случайные GREASE-значения в различные поля TLS Client Hello, чтобы затруднить
создание точных сигнатур DPI. Можно выбрать, какие именно компоненты модифицировать.
Рекомендуется использовать вместе с другими методами обхода, особенно при блокировках по
статическим сигнатурам (например, по списку шифров или расширений).

Пример: `--grease c,v,g,e` — максимальная рандомизация.

------
`--tls-split`

Разрезает TLS-запись на две отдельные записи, сохраняя полную валидность TLS-потока.
Это отличается от `--tlsrec`, который добавляет пустую запись (часто детектируемую DPI).
Разделение в середине SNI (`--tls-split 1+s+m`) может скрыть реальное имя хоста от
DPI, который анализирует только первую запись.

Можно комбинировать с `--auto` для применения только при обнаружении блокировки:
`--tls-split 1+s+m --auto=torst --timeout 2`
